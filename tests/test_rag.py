...
def test_corpus_answer_through_chat(mini_db, monkeypatch):
    ...
    assert t.kind == "corpus_answer" and not t.refused
    assert "IS 101" in t.text
...
def test_fallback_without_llm_credentials(mini_db, monkeypatch):
    _rag_env(monkeypatch, mini_db)
    from bis_assistant.assistant import answer
    r = answer("thick-walled bushes plain bearings specification")
    assert r["kind"] == "corpus_answer" and not r["refused"]
    assert r.get("rag_used_llm") is False
    assert "extractive" in r.get("rag_mode", "").lower()
    assert r.get("sources") and r.get("citations")
...
def test_chat_endpoint_serves_corpus(mini_db, monkeypatch, tmp_path):
    ...
    with TestClient(srv.app) as c:
        ...
        d = r.json()
        assert d["kind"] == "corpus_answer" and not d["refused"]
...
def test_regression_curated_flows_unaffected(mini_db, monkeypatch):
    _rag_env(monkeypatch, mini_db)
    from bis_assistant.assistant import answer
    r = answer("steel bottle")
    assert r.get("needs_info") and r["questions"]
    assert "IS 17803" in r["citations"][0]
    r2 = answer("vacuum insulated stainless steel water bottle flask 1 litre")
    assert "IS 17803" in r2["text"]


def test_full_text_still_refused_without_evidence(monkeypatch):
    ...
    assert r["refused"] and r["kind"] == "full_text"


def test_full_text_superseded_with_corpus_evidence(mini_db, monkeypatch):
    _rag_env(monkeypatch, mini_db)
    from bis_assistant.assistant import answer
