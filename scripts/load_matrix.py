"""Load matrix (Phase 8): sustained concurrent /chat with a registered key.

Usage: PYTHONPATH=src BIS_OPS_DB=/tmp/load.db python scripts/load_matrix.py [--n 200] [--c 20]
Reference env per plan: single instance. Asserts p95 <2000ms on admitted requests.
"""
...
def main() -> None:
    ...
    with TestClient(srv.app) as client:
        conn = srv._db()
        conn.execute("INSERT INTO users(key_hash, tier, created_at) VALUES (?,?,?)",
                     (hashlib.sha256(key.encode()).hexdigest(), "registered", "now"))
        conn.commit()
        conn.close()
        lat, codes = [], {}
        import time
    ...
    print(f"n={args.n} c={args.c} admitted={len(lat)} p50={p50:.0f}ms p95={p95:.0f}ms codes={codes}")
